...
public partial class Parser {
	...
	void Expression(out Expression exp) {
		Expression right = null, left = null; exp = null;
		OrTerm(out left);
		exp = left; 
		while (la.kind == 27) {
			Get();
	...
	static readonly bool[,] set = {
		...
		{x,x,x,T, x,x,T,T, T,x,T,x, x,x,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,T, T,x,x},
		{x,x,x,T, x,x,T,T, x,x,T,x, x,x,x,x, x,T,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,T, T,x,x},
