...
\subsection{Live Variables}\label{live_variables}
  ...
  occurences of variables. For instance, in $ CS(x,y) \defeq a . 0$ we would 
...
\end{Exa}  
  ...
  \end{figure}
	...
	Variables. An analysis class inherits from \textsf{AbstractVisitor} and 
	overrides its \texttt{Visit(Process p)} method. Since this is a forward 
	analysis the traversal is a little different than in \textit{Live 
	Variables}. Here, the property \textsf{VisitParentBeforeChildren} is set to 
	\texttt{true} on the \textsf{AbstractVisitor}. This makes sure that the tree 
