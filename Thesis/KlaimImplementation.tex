...
\section{Overview}
	...
	borrows the concept of \textit{tuplespaces}. A tuplespace is a collection 
	of tuples of data, these tuples can be read and removed from the tuplespace 
	...
	and its own tuplespace. The actions performed by the processes input, 
	output and read tuples of data from these tuplespaces. Unlike CCS, where 
	...
	The concept of data stores (or tuplespaces) and actions which operate on 
	...
	non-deterministic choice and action prefixing (albeit with different types 
	of actions than those built into the PLR). It also contains a fairly common 
...
\section{Syntax and semantics}\label{sec:klaim_syntax}
	...
	located processes as $l::P$. Parallel composition and non-deterministic 
	choice is represented the same way as in CCS, with $\mid$ and $+$  
...
\subsection{Pattern matching against tuples}\label{klaim_pattern_matching}
	...
	selecting a tuple from the tuplespace that fulfills these conditions:
	
	\begin{enumerate}
		\item The template tuple $t$ contains the same number of elements as the 
		candidate tuple $t'$ from the tuplespace.
	...
	in the tuplespace fulfills the conditions then the process is blocked until 
...
\section{Implementation}
	...
	replication and tuplespaces with their associated \textbf{in}, \textbf{out} 
...
\subsection{Replication}
	...
	\begin{enumerate}
		...
		\item Since \textbf{read} actions do not change the state of tuplespaces
		...
		action would have the same effect on the state of the tuplespaces, that 
...
\subsection{Tuplespaces}

	The tuplespaces of KLAIM are implemented in the KLAIM runtime library. The 
	...
	which represents its tuplespace. The \textbf{in}, \textbf{out} and 
	...
	As seen in these class descriptions, the implementation of tuplespaces is 
	...
	to access them, and the \code{Tuple} class implements the pattern matching.
	
	The tuples initially contained in each locality's tuplespace need to 
	...
	so the KLAIM compiler can then inject its tuplespace initialization code 
...
|| Finance :: * in(Confirmed, !item, !vendor, !amount, !person)@self
  ...
  \end{figure}
	...
	\begin{itemize}
		...
		with an \textbf{in} action, whenever an invoice (tuple) is available at 
		...
		matching tuple lands in their tuplespace. They read in the invoice and 
	...
	\textsf{self} tuplespace which implies that they own the data. 
