...
\subsection{Architecture}
	...
	invariably \textsf{Expression} nodes themselves.
 	...
 	two concrete descendants. One is \textsf{ChannelRestrictions} which 
 	restricts channels by name, provided that the names of channels to restrict 
 	are known at compile time. The other is \textsf{CustomRestrictions}, that 
 	calls a .NET method at runtime for every action and returns \textbf{true} it 
 	...
 	\textsf{Action} object from the PLR runtime library as a parameter and 
...
\subsection{Processing the syntax tree before compilation}\label{sec:visitor}
	...
	\textsf{Visit} method with itself as a parameter. An example of this is 
	shown in Figure~\ref{fig:visitor_example}. This technique, calling the 
	objects \textsf{Visit} methods that immediately calls the visitors 
	\textsf{Accept} methods is known as \textit{double dispatch}. This 
	...
	structure while calling the \textsf{Accept} method on each of its nodes. A 
	more detailed explanation of the \textit{Visitor} pattern and its benefits 
...
\section{CIL Structure of a Process Language Application}\label{cil_structure}
	...
	\TODO{Show some examples of CCS code => resulting .NET assembly}