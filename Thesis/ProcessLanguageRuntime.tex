...
\subsection{Architecture}
	...
	invariably \code{Expression} nodes themselves.
 	...
 	\code{IAction} object from the PLR runtime library as a parameter and 
 	...
 	and compiles down to a method that takes in an \code{IAction} runtime class 
 	...
 	.NET method that takes an \code{IAction} as a parameter and returns an 
 	\code{IAction} as well. This method can be written in any language 
 	available for .NET.
...
\subsection{Code generation}
	...
	located in the \code{System.Reflection.Emit} namespace. Before explaining 
...
\subsection{Debugging support}\label{debug_support}
	...
	the Common Intermediate Language format. The \code{System.Reflection.Emit} 
...
\section{Runtime Library}
	...
	\textbf{IParser} is an interface for parsers. It contains \code{Parse}
	...
	\code{ChannelSyncAction} implements the \code{IAction} interface as all 
	runtime actions must, it is a synchronous action and its \code{CanSyncWith} 
	method will only return true for other \code{ChannelSyncAction} instance 
	...
	actions are synchronized, the \code{Sync(IAction other)} method on both the 
	...
	in its own \code{Sync} method, as it knows that the receiving action will
...
\section{CIL Structure of a Process Language Application}\label{cil_structure}
	...
	has a parent, it does not and so it sets $B$'s parent to \code{null} before 
